...
def BSCR_Mort_Risk(baseLiabAnalytics, numOfLoB, Proj_Year, eval_date, mort_charge_table=BSCR_Config.Mort_Charge):
    ...
    key2.extend(key[i] for i in range(len(key) - 1))
...
def BSCR_Long_Risk_factor(BSCR_LOB, valDate, long_age = UI.long_age, long_dis = UI.long_dis, long_c = BSCR_Config.Long_Charge, long_f = UI.long_f):
    ...
    inpay = [val for key, val in long_c['Inpay'].items()]
    deferred = [val for key, val in long_c['Deferred'].items()]
    ...
    charge_inpay.extend(
        min(
            long_c['ult_c']['inpay'],
            (long_c['ult_c']["inpay"] - time_zero_inpay)
            / (long_age['ult_inpay'] - long_age["inpayment"][BSCR_LOB])
            * (
                1
                - int(valDate.strftime('%m')) / 12
                + i
                - 1
                + 1 * (int(valDate.strftime('%m')) == 12)
            )
            + time_zero_inpay,
        )
    ...
    for i in range(1,100):
        if long_age['deferred'][BSCR_LOB] > long_age['ult_def']:
            ...
            charge_def.append(
                min(
                    long_c['ult_c']['deferred'],
                    (long_c['ult_c']["deferred"] - time_zero_def)
                    / (long_age['ult_def'] - long_age["deferred"][BSCR_LOB])
                    * (
                        1
                        - int(valDate.strftime('%m')) / 12
                        + i
    ...
    return [
        i * long_dis[BSCR_LOB]['inpayment']
        + j * long_dis[BSCR_LOB]['deferred']
        for i, j in zip(charge_inpay, charge_def)
    ]           
...
#        if BSCR_LOB in Long_LOB:
    ...
    for t in range(0, Proj_Year + 1, 1):
        ...
        for idx in range(1, numOfLoB + 1, 1):
            ...
            if BSCR_LOB in Long_LOB and Risk_Type == 'Longevity':
                PVBE[BSCR_LOB][t] += abs(baseLiabAnalytics[idx].EBS_PVBE[t])
...
def BSCR_Morb_Charge(baseLiabAnalytics, numOfLoB, Proj_Year, morb_f = BSCR_Config.Morb_Charge, morb_d = UI.morbidity):
    ...
    for idx in range(1, numOfLoB + 1, 1):    
        ...
        if Risk_Type == 'Morbidity & Disability' and BSCR_LOB in Morb_LOB:
            PVBE[BSCR_LOB]    = {}
            Premium[BSCR_LOB] = {}
...
def BSCR_PC_Res_Charge(baseLiabAnalytics, numOfLoB, Proj_Year, regime = "Current", method = "Bespoke", pc_f = BSCR_Config.Reserve_Risk_Charge, pc_m = UI.PC_mapping, pc_cor = BSCR_Config.pc_cor):
    ...
    for idx in range(1, numOfLoB + 1, 1):  
        ...
        if baseLiabAnalytics[idx].LOB_Def['Agg LOB'] == 'PC': 
            PVBE[idx] = {t: 0 for t in range(0, Proj_Year + 1, 1)}
    # Sum up BLOB PVBE               
    for t in range(0, Proj_Year + 1, 1):
        for idx in range(1, numOfLoB + 1, 1):                                             
            if baseLiabAnalytics[idx].LOB_Def['Agg LOB'] == 'PC':            
    ...
    Risk_charge = {
        each_BLOB: BLOB_PVBE.loc[:, each_BLOB] * pc_f[method][each_BLOB]
...
def BSCR_FI_Risk_Charge(portInput, AssetAdjustment, valDate):
    ...
    return {
        'Agg': [
            BSCR_FI_EA_Risk_Charge_Agg + BSCR_FI_AA_Risk_Charge_Agg,
            (BSCR_FI_EA_Risk_Charge_Agg + BSCR_FI_AA_Risk_Charge_Agg)
            / FI_Exposure_Agg,
        ],
        'LT': [
            BSCR_FI_EA_Risk_Charge_LT + BSCR_FI_AA_Risk_Charge_LT,
            (BSCR_FI_EA_Risk_Charge_LT + BSCR_FI_AA_Risk_Charge_LT)
            / FI_Exposure_LT,
        ],
        'GI': [
            BSCR_FI_EA_Risk_Charge_GI + BSCR_FI_AA_Risk_Charge_GI,
            (BSCR_FI_EA_Risk_Charge_GI + BSCR_FI_AA_Risk_Charge_GI)
            / FI_Exposure_GI,
        ],
    }
